...
public class GatecoinExchangeTest extends ExchangeTest {
	@Override
	@Before
	public void setUp() throws Exception {
		testExchange = new GatecoinExchange(10000);
	}
	...
	public void testParseTicker() {
		try {
			...
			JsonNode node = (new ObjectMapper().readTree(new File("src/test/json/gatecoin-ticker.json")));
			String lastValue = testExchange.parseTicker(node, pair);
			Assert.assertEquals("544.12", lastValue);
		} catch (IOException e) {
			e.printStackTrace();
			Assert.fail();
		}
