...
public class LivecoinExchangeTest extends ExchangeTest {
	@Override
	@Before
	public void setUp() throws Exception {
		testExchange = new LivecoinExchange(10000);
	}
	...
	public void testParseTicker() {
		try {
			...
			JsonNode node = (new ObjectMapper().readTree(new File("src/test/json/livecoin-ticker.json")));
			String lastValue = testExchange.parseTicker(node, pair);
			Assert.assertEquals("792.37", lastValue);
		} catch (IOException e) {
			e.printStackTrace();
			Assert.fail();
		}
