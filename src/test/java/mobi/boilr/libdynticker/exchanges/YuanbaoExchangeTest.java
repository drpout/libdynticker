...
public class YuanbaoExchangeTest extends ExchangeTest {
	@Override
	@Before
	public void setUp() throws Exception {
		testExchange = new YuanbaoExchange(1000);
	}
	...
	public void testParseTicker() {
		try {
			...
			JsonNode node = (new ObjectMapper().readTree(new File("src/test/json/yuanbao-ticker.json")));
			String lastValue = testExchange.parseTicker(node, pair);
			Assert.assertEquals("5579.99", lastValue);
		} catch(IOException e) {
			Assert.fail();
		}
	}
