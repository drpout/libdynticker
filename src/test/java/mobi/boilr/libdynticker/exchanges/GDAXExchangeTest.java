...
public class GDAXExchangeTest extends ExchangeTest {
	@Override
	@Before
	public void setUp() throws Exception {
		testExchange = new GDAXExchange(10000);
	}
	...
	public void testParseTicker() {
		try {
			...
			JsonNode node = (new ObjectMapper().readTree(new File("src/test/json/gdax-ticker.json")));
			String lastValue = testExchange.parseTicker(node, pair);
			Assert.assertEquals("603.90000000", lastValue);
		} catch (IOException e) {
			e.printStackTrace();
			Assert.fail();
		}
